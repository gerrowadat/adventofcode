...
class Line(object):
  ...
  def contains(self, point):
    """Returns the number of steps from start of line to this point, or None if the point ins't on the line.."""
    if self.direction in ('U', 'D'):
      if self.start.x != point.x:
        return None
      if is_between(self.start.y, self.end.y, point.y):
        return abs(self.start.y - point.y)
    if self.direction in ('L', 'R'):
      if self.start.y != point.y:
        return None
      if is_between(self.start.x, self.end.x, point.x):
...
def distance_to_point(wire, p):
  """wire (str): string representation of a wire.
     p (Point): Point on this line where there is an intersection."""
  wire_lines = get_lines(wire.split(','))
  step_count = 0
...
for p in intersections:
  wire1_to_p = distance_to_point(wire1, p)
  wire2_to_p = distance_to_point(wire2, p)
  print('Combined Steps to %s: (%d + %d): %d' % (p, wire1_to_p, wire2_to_p, (wire1_to_p + wire2_to_p)))
  if wire1_to_p + wire2_to_p < shortest or shortest is None:
    shortest = wire1_to_p + wire2_to_p
    
print('Fewest combined steps: %d' % (shortest, ))
